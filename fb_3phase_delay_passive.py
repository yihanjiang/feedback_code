...
class AE(torch.nn.Module):
    ...
    def forward(self, input, fwd_noise, fb_noise):
        ...
        x_p1_rec_  = self.power_constraint(x_p1_rec)

        x_p1_fb   = x_p1_rec_  + fb_noise[:,:, 0].view(self.args.batch_size, self.args.block_len, 1)
        ...
        x_p2_rec_  = self.power_constraint(x_p2_rec)

        x_p2_fb   = x_p2_rec_  + fb_noise[:,:, 1].view(self.args.batch_size, self.args.block_len, 1)
        ...
        x_p3_rec_      = self.power_constraint(x_p3_rec)

        x_p3_fb   = x_p3_rec_  + fb_noise[:,:, 2].view(self.args.batch_size, self.args.block_len, 1)
        ...
        # Decoder
        # if self.training:
        #     print 'train', torch.mean(codes_adjust), torch.std(codes_adjust)
        # else:
        #     code = torch.cat([x_p1_norm,x_p2_norm,x_p3_norm], dim=2)
        #     print 'test', torch.mean(code), torch.std(code)
