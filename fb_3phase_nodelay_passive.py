...
def get_args():
    ...
    parser.add_argument('-snr_test_end', type=float, default=3.0)
    parser.add_argument('-snr_points', type=int, default=3)

    parser.add_argument('-channel_mode', choices=['normalize', 'lazy_normalize', 'tanh'], default='lazy_normalize')

    parser.add_argument('-fb_dropout', choices=['0.0', '0.1', '0.2', '0.5', 'every_two'], default='0.0')
    ...
    parser.add_argument('--no_weight_allocation', action='store_true', default=True,
...
class Power_reallocate(torch.nn.Module):
    ...
    def forward(self, inputs, phase = -1, index = -1):
        ...
        if phase == -1:
            ...
            res = self.wt[index, phase] * inputs.view(self.args.batch_size, 1, 1)
            res = res.view((self.args.batch_size, 1, 1))
...
class AE(torch.nn.Module):
    def __init__(self, args):
        ...
        self.enc_p2_linear    = torch.nn.Linear(args.enc_num_unit, 1) # Generate two codewords per cell.
        ...
        self.enc_p3_linear    = torch.nn.Linear(args.enc_num_unit, 1) # Generate two codewords per cell.
    ...
    def forward(self, input, fwd_noise, fb_noise):
        ...
        for idx in range(input.shape[1]):
            ...
            if not self.args.no_weight_allocation:
                if not self.training:
                    x_tmp_p1  = self.total_power_reloc(x_tmp_p1, 0, idx)

            x_p1_rec  = x_tmp_p1  + fwd_noise[:,idx,0].view(self.args.batch_size, 1, 1)

            x_p1_fbenc  = self.power_constraint(x_p1_rec)
        ...
        for idx in range(input.shape[1]):
            ...
            if not self.args.no_weight_allocation:
                if not self.training:
                    x_tmp_p2  = self.total_power_reloc(x_tmp_p2, 1, idx)

            x_p2_rec  = x_tmp_p2  + fwd_noise[:,idx, 1].view(self.args.batch_size, 1, 1)

            x_p2_fbenc  = self.power_constraint(x_p2_rec)
        ...
        for idx in range(input.shape[1]):
            ...
            if idx == 0:
                ...
                x_tmp_p3         = self.enc_act(self.enc_p3_linear(x_fwd_p3))
            ...
            if not self.args.no_weight_allocation:
                if not self.training:
                    x_tmp_p3  = self.total_power_reloc(x_tmp_p3, 2, idx)

            x_p3_rec  = x_tmp_p3  + fwd_noise[:,idx, 2].view(self.args.batch_size, 1, 1)

            x_p3_fbenc  = self.power_constraint(x_p3_rec)
